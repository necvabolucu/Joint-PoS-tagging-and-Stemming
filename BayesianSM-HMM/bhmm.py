...
#                return  tagProb[i],stemProb[i]
    ...
    def __compute_label_probabilities(self, blanket):
        ...
        _, previous_previous_label,previous_label, following_label,following_following_label, current_observation,current_stem,current_affix = blanket
...
#        print(probabilities,tagProb,stemProb,affixProb)
        ...
        with open(self.fout, "w", encoding="utf8") as f:
            for i in range(len(self.data)):
                ...
                for j in range(len(self.sequences[i])):
                    labeled_unit.append("%s&%s+%s/%s" % (self.data[i][j],self.stems[i][j],self.affixs[i][j],self.sequences[i][j]))
    ...
    def __change_sample(self, blanket, i):
        ...
        i -- add or remove
        """
        current_label,previous_previous_label, previous_label, following_label, following_following_label,current_observation,current_stem,current_affix = blanket
        ...
        change_count(self.affixEmission, current_affix, current_label, i)
